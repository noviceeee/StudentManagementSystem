...
<%
	...
	name = checkStr(strName);
	sex = checkStr(strSex);
	age = checkInt(strAge);
	height = checkInt(strHeight);
	weight = checkInt(strWeight);
	...
	if(strUpdate != null && strUpdate.equals("true"))update=true;
	...
	<div>
		...
		<form action="Update.jsp">
			学号<input type="text" name="id" value=<%=id%> readonly> 姓名<input
				...
				type="text" name="sex" value=<%=sex%>><br> <br>
			年龄<input type="text" name="age" value=<%=age%>> 身高<input
				type="text" name="height" value=<%=height%>> 体重<input
				type="text" name="weight" value=<%=weight%>><br> <br>
